...
EventClustering::EventClustering(){
  ...
  for (int i = 0; i < Common::noDSSD;i++){
    ...
    hname[0] = '\0';
    sprintf(hname,"implantDSSD%ixy",i);
    implantDSSDxy[i] = new TH2I(hname,";x;y",128,0,128,128,0,128);
  }
...
void EventClustering::ProcessMaps(){
  ...
  if(implantMap.size() > 0){
    ...
    for(int i = 0; i<Common::noDSSD ;i++){	
      ...
      Etest = dssdImplantLists[i][0].front().GetStrip();
      Etest2 = dssdImplantLists[i][1].front().GetStrip();
...
short EventClustering::ImplantStoppingLayer(){
  ...
  for (int stoppingLayer = 0; stoppingLayer < Common::noDSSD; stoppingLayer++){
    ...
    if(dssdImplantLists[stoppingLayer][0].size() > 0 && dssdImplantLists[stoppingLayer][1].size() > 0){
      ...
      if(stoppingLayer < (Common::noDSSD -1)){
	...
	//If not the first detector, check for upstream events. Require at least one side of an upstream detector to fire in same event window
...
#ifdef DEB_IMPLANT_STOPPING
      if(!downstreamEvents) std::cout << "No downstream events and ";
      else if(downstreamEvents) std::cout << "Downstream events and ";
      if(upstreamEvents) std::cout << "upstream events in DSSD " << stoppingLayer << std::endl;
      else if(!upstreamEvents) std::cout << "no upstream events in DSSD " << stoppingLayer << std::endl;
#endif
  ...
  if(clusterLists[dssd][0].size() > 0 && clusterLists[dssd][1].size() > 0){
	...
	if(abs(clusterSide0It->GetEnergy()-clusterSide1It->GetEnergy()) <= equalEnergyRange){
	  ...
	  if((clusterSide0It->GetTimestampDifference(clusterSide1It->GetTimestampMin())<4000) ||
	    ...
	    //Set the x and y multiplicity of the clusters. 0 = n-side = y. 1 = p-side = x
...
#ifdef OLD_OUTPUT
	    MergerOutputOld pairedCluster(*clusterSide1It, *clusterSide0It);
	    outputEvents.emplace(pairedCluster.GetTimestamp(),pairedCluster);
#endif
#ifdef NEW_OUTPUT
	    MergerOutputNewTrial pairedCluster(*clusterSide1It, *clusterSide0It);
	    outputEvents.emplace(pairedCluster.GetTimestamp(),pairedCluster);
#endif
#ifdef MERGER_OUTPUT
	    MergerOutput pairedCluster(*clusterSide1It, *clusterSide0It);
	    outputEvents.emplace(pairedCluster.GetTimestamp(),pairedCluster);
#endif
	    ...
	    if(equalEnergyRange == decayEnergyDifference){
	      ...
	      implantDSSDxy[dssd]->Fill(clusterSide1It->GetStrip(),clusterSide0It->GetStrip());
	    }
...
void EventClustering::CloseClustering(){
  ...
  std::cout << "Percentage of implant event windows being written to file: " << ((double)implantTimeMatchCounter/((double)implantWindowCounter))*100 << std::endl;
