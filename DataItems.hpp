...
public:
  ...
  void BuildScalerItem(std::pair < unsigned int, unsigned int> inData);

  void BuildTimestamp(unsigned long MSB);

  void SetTimestamp(unsigned long newTimestamp);
  void SetTimestampLSB(unsigned long value);
...
public:
  ...
  void SetTimestampFull(unsigned long fullTimstamp);
  void SetCorrScaler(unsigned long corrScaler);
};
...
public:
  ...
  ScalerOutput();
  ScalerOutput(InformationDataItem & informationDataItem);
